...
/*router.get('/index_category', function (req, res) {
  setTimeout(function () {
    ...
    // const data = required(URL.parse(req.url,true).query,'index_category')
    res.send({code: 0, data})
  }, 300)
})*/
router.get('/index_category', function (req, res) {
...
搜索餐馆信息
 */
router.get('/search_shops',async function(req, res, next){
  ...
  try{
    Shops.find({name: eval('/' + keyword + '/gi')},'-_id', function (err,data) {
      if (err) {
        res.json({
          code: '1',
          msg: err.message,
